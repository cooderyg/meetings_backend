 ...
 * 애플리케이션 전용 예외 클래스
 *
 * ErrorCode enum을 기반으로 표준화된 에러 응답을 생성합니다.
...
export class AppException extends HttpException {
   ...
   * AppException 생성자
   *
   * @param errorCode - 에러 코드 (ErrorCode enum 값)
   * @param options - 추가 옵션 (메시지, 상세정보 등)
   */
  constructor(errorCode: ErrorCode, options?: AppExceptionOptions) {
    // 에러 코드 검증 (개발 환경)
    AppException.validateErrorCode(errorCode);

    // ERROR_DEFINITIONS에서 에러 정의 조회
    ...
    this.code = errorCode; // enum 값이 그대로 코드가 됨 (예: 'AUTH_001')
  ...
  /**
   * 에러 코드 검증 정적 메서드
   * @param errorCode 검증할 에러 코드
   */
  private static validateErrorCode(errorCode: ErrorCode): void {
    if (process.env.NODE_ENV === 'development') {
      if (!Object.values(ErrorCode).includes(errorCode)) {
        console.warn(
          `[AppException] Invalid error code: ${errorCode}\n` +

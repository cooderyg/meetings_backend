...
export class MeetingPermissionGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const requiredAction = this.reflector.get<Action>('meeting-action', context.getHandler());
    if (!requiredAction) {
    ...
    try {
      ...
      if (!hasPermission) {
        throw new AppException(ERROR_CODES.AUTH_FORBIDDEN, {
          ...
          details: { resource: 'meeting', action: requiredAction, resourceId: meetingId }
        });
      }
      ...
      throw new AppException(ERROR_CODES.AUTH_FORBIDDEN, {
        ...
        details: { resource: 'meeting', action: requiredAction, error: error.message }
      });
    }
  }
}